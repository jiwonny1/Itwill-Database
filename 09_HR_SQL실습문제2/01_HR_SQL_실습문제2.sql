...
*/  
SELECT CONCAT(FIRST_NAME,' '||LAST_NAME) AS NAME, ROUND(SALARY*(1 + 0.123)) AS Increase_Salary
FROM EMPLOYEES
...
*/
SELECT CONCAT(FIRST_NAME,' '||LAST_NAME) AS NAME, SALARY,
            DECODE(commission_pct, null, 'Salary only', TRUNC(SALARY*(1 + commission_pct)) ) AS DECODE_SALARY
FROM EMPLOYEES     
...
*/  
SELECT JOB_ID, TO_CHAR(SUM(SALARY),'$999,999,999') AS SUM_SALARY,
                TO_CHAR(AVG(SALARY),'$999,999,999')AS AVG_SALARY,
                TO_CHAR(MAX(SALARY),'$999,999,999')AS MAx_SALARY,
                TO_CHAR(MIN(SALARY),'$999,999,999')AS MIN_SALARY
FROM EMPLOYEES
...
*/
SELECT 'HR-Company',CONCAT(FIRST_NAME,' '||LAST_NAME) AS NAME, E.JOB_ID ,D.DEPARTMENT_NAME
            ,L.CITY
FROM DEPARTMENTS D, EMPLOYEES E, LOCATIONS L
WHERE D.DEPARTMENT_ID = E.DEPARTMENT_ID
  AND L.LOCATION_ID = D.LOCATION_ID
  AND INITCAP(L.CITY) = 'Oxford'
;
